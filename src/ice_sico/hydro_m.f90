...
!! @section Copyright
!!
!! Copyright 20xx-2016 Sebastian Beyer...
!!
!! @section License
!!
...
!-------------------------------------------------------------------------------
!  This is a generated file from gen_sico_coupling.sh   
!  In it the complete hydrology from libhydro is merged 
!  into one single file and the precision_m module is   
!  replaced by the SICOPOLIS sico_types.                

#define libhydro_VERSION_MAJOR 2
#define libhydro_VERSION_MINOR 3
#define libhydro_VERSION_PATCH 0
...
contains
  ...
  subroutine BoxBlur (source, filtered, r)
    ...
    real(wp), allocatable                :: source_transp(:,:)   ! source transposed
    real(wp), allocatable                :: filtered_transp(:,:)   ! filtered transposed
  ...
  subroutine fastGauss (source, filtered, r)
    ...
    real(wp), allocatable                :: tmpfilter1(:,:)   ! tmp array to store intermediate results
    real(wp), allocatable                :: tmpfilter2(:,:)   ! tmp array to store intermediate results
...
!-------------------------------------------------------------------------------
module priority_flood_m

  use sico_types_m, only: wp

  use priority_queue_m, only: gridCell, queue
...
contains
  ...
  subroutine pf_flood (dem, nx,ny, flatmask, tiltflats, nn_o)
    real(wp), dimension(:,:), intent(inout)       :: dem
    integer, intent(in)                           :: nx,ny
    ...
    real(wp)                                      :: tiltinc        ! increment for tilting
    type (queue)                                  :: pqopen
    type (gridCell)                               :: cell
...
end module priority_flood_m

!-------------------------------------------------------------------------------
module balance_flux_m
...
contains
  ...
  recursive subroutine dparea_war(i, j, flow_num, nx, ny, x, y, &
    ...
    real(wp),dimension(nx,ny),intent(in) :: srf     !! surface to compute the flow direction
    real(wp),dimension(nx,ny),intent(in) :: accum   !! accumulation/source
    real(wp),dimension(nx,ny),intent(inout) :: flux !! local computed flux

    real(wp), dimension(9) :: pp,delev
    real(wp), dimension(9) :: dd
 ...
 ! print *, "Start at ", i,j, flux(i,j)
          ...
          do ii=i-1,i+1
            ...
            if(count .ne. 5) then
              ...
              if (ii > 1 .and. ii < nx .and. jj > 1 .and. jj < ny) then
                  ...
                  do iii=ii-1,ii+1
                    ...
                    if (flow_num == 4) then
                      if(count2 .ne. 5 .and. ((count2/2)*2) == count2) then
                        delev(count2) = max(0.0_wp,(srf(ii,jj) - srf(iii,jjj)))
                      ...
                      if(count2 .ne. 5) then
                        delev(count2) = max(0.0_wp,(srf(ii,jj) - srf(iii,jjj))) * dd(count2)
                        sumd = sumd + delev(count2)
                      end if
                ...
                if (sumd == 0.0_wp .and. srf(ii,jj) > 0.0_wp) then
                  sumdcount = 0
                  ...
                  do jjj=jj-1,jj+1
                    do iii=ii-1,ii+1
                      ...
                      if (flow_num == 4) then
                        if(count2 .ne. 5 .and. ((count2/2)*2) == count2) then
                          if (srf(iii,jjj) == srf(ii,jj)) then
                            ! pp(count2) = 1.0_wp/sumdcount !ORG
                            pp(count2) = 1.0_wp/real(sumdcount,KIND=wp)
                          end if
                        end if
                        !8 flow directions
                      else if (flow_num == 8) then
                    ...
                    do iii=ii-1,ii+1
                      ...
                      if (flow_num == 4) then
                        if(count2 .ne. 5 .and. ((count2/2)*2) == count2) then
                          ...
                          if (sumd > 0.0_wp) pp(count2) = delev(count2)/sumd
                        end if
                      else if (flow_num == 8) then
                        if(count2 .ne. 5) then
                          !pp(count2) = delev(count2)/sumd !ORG
                ...
                if (pp(dirpp_war(count)) > 0.0_wp) then
  ...
  recursive subroutine dparea_tar(i, j, flow_num, nx, ny, x, y, &
    ...
    integer,intent(in) :: ny  !! number of grid points nsn
    !
    real(wp),dimension(nx),intent(in) :: x !! grid spacing x
    real(wp),dimension(ny),intent(in) :: y !! grid spacing y
    !
    integer, dimension(nx,ny),intent(in) :: imask    !!
    real(wp),dimension(nx,ny),intent(in) :: srf     !! surface to compute the flow direction
    real(wp),dimension(nx,ny),intent(in) :: accum   !! accumulation/source
    real(wp),dimension(nx,ny),intent(in) :: flowdir !! flowdir
    !
    real(wp),dimension(nx,ny),intent(inout) :: flux !! local computed flux
    ! --
    real(wp), dimension(9) :: pp
    integer :: ii,jj,count,sector
    real(wp) :: alpha2
    real(wp) :: alpha
    real(wp) :: dx,dy
    real(wp), parameter :: pi4 = pi/4.0_wp
      ...
      if (flux(i,j) < 0.0_wp) then
        ...
        do jj=j-1,j+1
          do ii=i-1,i+1
            ...
            ! flowdir < 0 is noflow
            if(count .ne. 5 .and. flowdir(ii,jj) >= 0.0_wp) then
              ...
              if(alpha > 2.0_wp *pi) alpha = alpha - 2.0_wp * pi
              ...
              alpha2 = (real(sector,wp) * pi4) - alpha
              ! FIX
              if(alpha2 < 0.0_wp) alpha2 = alpha2 + 2.0_wp *pi
              ...
              if (sector .ne. 8) then
                ...
                pp(sector + 1) = 1.0_wp - pp(sector)

              else

                ! pp(sector) = alpha2/45.0  ! check this TK because we now have 0 2Pi
                pp(sector) = alpha2/pi4  !
                pp(1) = 1.0_wp - pp(sector)
              ...
              if (pp(dirpp_tar(count)) > 0.0_wp ) then
  ...
  subroutine flow_dir(nx, ny, x, y, imask, srf, srf_x, srf_y, theta)
    ...
    integer,dimension(nx,ny),intent(in) :: imask     !!
    real(wp),dimension(nx,ny),intent(in) :: srf !! surface to compute the flow direction
    real(wp),dimension(nx,ny),intent(out) :: srf_x     !!
    real(wp),dimension(nx,ny),intent(out) :: srf_y     !!
    real(wp),dimension(nx,ny),intent(out) :: theta     !! flow direction
  ...
  subroutine tar_flow_dir(nx, ny, x, y, imask, srf, srf_x, srf_y, theta)
    ...
    integer :: i,j
    integer :: max_pos,ct
    real(wp) :: a1, rg, max_val
    real(wp) :: r(8), s(8), dx,dy

    ! af in table 6 Tarboton 1997
    real(wp), parameter :: af(8) = &
        (/ 1.0_wp, -1.0_wp, 1.0_wp, -1.0_wp, 1.0_wp, -1.0_wp, 1.0_wp, -1.0_wp /)

    ! ac in table 6 Tarboton 1997
    real(wp), parameter :: ac(8) = &
        (/ 0.0_wp, 1.0_wp, 1.0_wp, 2.0_wp, 2.0_wp, 3.0_wp, 3.0_wp, 4.0_wp /)
    ...
    do j = 2,ny-1
      do i = 2,nx-1
        ...
        if ( imask(i,j) > 0 ) then
          ...
          a1 = r(max_pos)
          rg = r(max_pos) * af(max_pos) + ac(max_pos) * pi / 2.0_wp ! eq. 6 in Tarboton 1997

          if (max_val .lt. 0.0_wp) then
            rg = -9999 ! noflow
          else
            ! tkleiner adjust to the different i,j orientation
            if (rg > 0.0_wp)  rg = 2.0_wp*pi - rg
          end if
  ...
  subroutine tar_rs(e0,e1,e2,d1,d2,r,s)
    ...
    real(wp), intent(in) :: e0,e1,e2
    real(wp), intent(in) :: d1,d2 !
    real(wp), intent(out) :: r,s

    real(wp) :: s1, s2
    real(wp), save :: r_lim, con
...
end module balance_flux_m

!-------------------------------------------------------------------------------
module modelstate_flux_m
  ...
  type :: modelstate_flux_t
    real(wp), dimension(:,:), allocatable :: vflux  ! vector flux
    real(wp), dimension(:,:), allocatable :: sflux  ! scalar flux
    real(wp), dimension(:,:), allocatable :: vfluxX
    real(wp), dimension(:,:), allocatable :: vfluxY
    real(wp), dimension(:,:), allocatable :: dir    ! direction of flux
    real(wp), dimension(:,:), allocatable :: bwat   ! water layer thickness
...
end module modelstate_flux_m

!-------------------------------------------------------------------------------
module modelstate_geom_m
  ...
  type :: modelstate_geom_t
    ...
    real(wp)                              :: dx
    real(wp)                              :: dy
    real(wp), dimension(:),   allocatable :: x  ! vector containing x values
    real(wp), dimension(:),   allocatable :: y  ! vector containing y values
    integer,  dimension(:,:), allocatable :: icemask
    real(wp), dimension(:,:), allocatable :: usurf
    real(wp), dimension(:,:), allocatable :: thk
    real(wp), dimension(:,:), allocatable :: topg
    
    real(wp), dimension(:,:), allocatable :: bmelt
    real(wp), dimension(:,:), allocatable :: temppabase
  contains
...
end module modelstate_geom_m

!-------------------------------------------------------------------------------
module modelstate_pot_m
  ...
  type :: modelstate_pot_t
    ...
    real(wp), dimension(:,:), allocatable :: pot_smooth
    real(wp), dimension(:,:), allocatable :: pot_filled
    real(wp), dimension(:,:), allocatable :: pot_diff  !! potential difference (amount filled, e.g. 'lake volume')
...
module hydro_modelstate_m
  ...
  use modelstate_geom_m, only: modelstate_geom_t
  use modelstate_pot_m, only: modelstate_pot_t
  use modelstate_flux_m, only: modelstate_flux_t
...
module hydro_conf_m
  ...
  type hydro_conf_t
     ...
     real(wp)           :: global_supply !! water supply globally set from cmd line [m/a]
     logical            :: read_bmelt
     ...
     real(wp)           :: filter_len !! filter length for hyd. potential [m]
     real(wp)           :: rho_freshwater !! used for hydraulic potential
     real(wp)           :: rho_seawater !! used for icemask calculation
     real(wp)           :: rho_ice
     real(wp)           :: gravity_const
     real(wp)           :: min_thickness !! minimum ice thickness to be considered in icemask
...
contains
  ...
  subroutine init_config(self)
    ...
    self%global_supply = 0.0_wp
    self%read_bmelt = .true.
    self%ncFormat = 'test'
    self%avoid_frz = .false.
    self%filter_len = 0.0_wp
    self%rho_freshwater = 1000.0_wp
    self%rho_seawater = 1025.0_wp
    self%rho_ice = 917.0_wp
    self%gravity_const = 9.81_wp
    self%min_thickness = 1.0_wp
    self%comp_mask = .true.
...
!-------------------------------------------------------------------------------
module barnes_wrapper_m

  use sico_types_m, only: sp, wp
...
!-------------------------------------------------------------------------------
!> Main module: Routing scheme for the basal meltwater.
!<------------------------------------------------------------------------------
module hydro_m

! /* #include "libhydro_version.h" */
  ...
  use modelstate_pot_m, only: modelstate_pot_t
  use modelstate_flux_m, only: modelstate_flux_t
  use fortfilt_m, only: BoxBlur
  use priority_flood_m, only: pf_flood
  use balance_flux_m, only: dparea_war, dparea_tar, flow_dir, tar_flow_dir
  use hydro_conf_m, only: hydro_conf_t
  use barnes_wrapper_m, only: barnes_resolve_flats
...
contains
  subroutine hydro_potential (potential, geometry, RHO_FRESHWATER, RHO_ICE, G)
    ...
    real(wp), intent(in)                      :: RHO_FRESHWATER
  ...
  subroutine smooth_potential_meters (potential, fltlen_meters, dx)
    ...
    !! smooth_potential_pixel
    class(modelstate_pot_t), intent(inout)     :: potential
    real(wp), intent(in)                       :: fltlen_meters
  ...
  subroutine resolve_flats_random (potential, nneighbors)
    ...
    integer                                    :: nx, ny
    integer :: cnt,i,j,l
    real(wp) :: grad_x,grad_y,grad,rand01,old
    ...
    do j = 2,ny-1
       do i = 2,nx-1
          ...
          grad_y = ( potential%pot_filled(i,j+1) - potential%pot_filled(i,j-1) )
             ...
             if (grad <= 0.0_wp) then
                cnt = cnt + 1
                if (grad_x == 0.0_wp) then
                   old = potential%pot_filled(i+1,j)
                   call random_number(rand01)
                   potential%pot_filled(i+1,j) = potential%pot_filled(i+1,j) + (rand01 - 0.5_wp)*1e-3_wp ! +/- 5*10^-7
                end if

                if (grad_y == 0.0_wp) then
                   old = potential%pot_filled(i,j+1)
                   call random_number(rand01)
                   potential%pot_filled(i,j+1) = potential%pot_filled(i,j+1) + (rand01 - 0.5_wp)*1e-3_wp ! +/- 5*10^-7
                end if
             end if
       end do
    end do
  ...
  subroutine generateIceMask (geometry, RHO_ICE, RHO_SEAWATER, min_thickness)
    ...
    real(wp), intent(in)                        :: RHO_ICE
    real(wp), intent(in)                        :: RHO_SEAWATER  !! seawater density
    real(wp), intent(in)                        :: min_thickness !! minimum ice thickness to be considered
    real(wp), dimension(:,:), allocatable       :: H_eq     ! equilibrium height

    allocate(H_eq(geometry%nx, geometry%ny))
    H_eq = ( 1.0_wp - RHO_ICE/RHO_SEAWATER ) * geometry%thk
    geometry%icemask = 0
  ...
  subroutine ensure_positive_supply(geometry)
    ...
    geometry%bmelt = max (geometry%bmelt, 0.0_wp)
  ...
  subroutine balance_flux (flux, potential, geometry, method, mask)
    ...
    integer                                    :: dim(2)
    integer                                    :: nx, ny
    real(wp), dimension(:,:), allocatable    :: pot_x     !! d pot dx
    real(wp), dimension(:,:), allocatable    :: pot_y     !! d pot dy
    integer                                  :: i,j
  ...
  subroutine flux_correction (flux, potential, geometry, method, mask)
    ...
    integer                                    :: dim(2)
    integer                                    :: nx, ny
    real(wp), dimension(:,:), allocatable    :: pot_x     !! \partial pot / \partial x
    real(wp), dimension(:,:), allocatable    :: pot_y     !! \partial pot / \partial y
    integer                                  :: i,j
    ...
    do j = 2,ny-1
       do i = 2,nx-1
        ...
        if (mask(i,j) > 0) then
          ...
          if (corone <= 0.0_wp ) then
            ! we take the flow direction es undifined
            corfac = 1.0_wp
          else
            dx = 0.5_wp*(geometry%x(i+1)-geometry%x(i-1))
            dy = 0.5_wp*(geometry%y(j+1)-geometry%y(j-1))
            !if (dx /= dy ) then
            if ( abs(dx - dy) > 1e-2_wp ) then
  ...
  subroutine flux2bwat(flux, potential, geometry, mask)
    ...
    integer :: i,j
    real(wp) :: pot_x,pot_y,grad
    ! "Water at the glacier bed", J.F. Nye
    ! Weertman_Birchfield_1983
    real(wp), parameter :: visc_water = 1.8e-3_wp ! Pa*s = 1.8x10^-2 Poise
    integer :: jstart,jend
    integer                                    :: dim(2)
    integer                                    :: nx, ny
    dim = shape(potential%pot)
    ...
    do j=jstart,jend
      do i = 2,nx-1
        ...
        if (mask(i,j) > 0) then
          ...
          ! if(cflux(i,j)< 0.0_wp) print *,i,j,cflux(i,j)," error"
          if(grad > 0.0_wp .and. flux%vflux(i,j) > 0.0_wp .and. mask(i,j) > 0) then
            flux%bwat(i,j) = ( 12.0_wp * visc_water * flux%vflux(i,j) / grad )**(1.0_wp/3.0_wp)
          else
            ! ! should be checked
  ...
  subroutine hydro_gen_conf (hydro, method, flatresolution, &
    ...
    real(wp), intent(in),optional          :: filter_len
    real(wp), intent(in),optional          :: rho_seawater
    real(wp), intent(in),optional          :: rho_freshwater
    real(wp), intent(in),optional          :: rho_ice
    real(wp), intent(in),optional          :: min_thickness
  ...
  subroutine hydro_init (hydro, x, y)
    ...
    type (hydro_t), intent(inout)    :: hydro
    real(wp), intent(in), dimension(:):: x !! x coordinates
    real(wp), intent(in), dimension(:):: y !! y coordinates
  ...
  subroutine hydro_update (hydro)
    ...
    where ( hydro%state%potential%pot_diff > 1.0_wp ) hydro%state%flux%bwat = 10.0_wp
  ...
  subroutine hydro_get_bwat (hydro, bwat)
    ...
    real(wp), intent(out), dimension(:,:) :: bwat !! basal water layer thickness (meter)
  ...
  subroutine hydro_get_sflux (hydro, sflux)
    ...
    type(hydro_t),intent(in)              :: hydro
    real(wp), intent(out), dimension(:,:) :: sflux !! scalar (volume) flux (meter^3/sec)
  ...
  subroutine hydro_set_supply (hydro, supply)
    ...
    type(hydro_t),intent(inout)          :: hydro
    real(wp), intent(in), dimension(:,:) :: supply !! water supply (meter per second)
  ...
  subroutine hydro_set_temppabase (hydro, temppabase)
    ...
    type(hydro_t),intent(inout)          :: hydro
    real(wp), intent(in), dimension(:,:) :: temppabase !! basal temperature rel. to pmp
  ...
  subroutine hydro_set_thk (hydro, thk)
    ...
    type(hydro_t),intent(inout)          :: hydro
    real(wp), intent(in), dimension(:,:) :: thk !! ice thickness (meter)
  ...
  subroutine hydro_set_topg (hydro, topg)
    ...
    type(hydro_t),intent(inout)          :: hydro
    real(wp), intent(in), dimension(:,:) :: topg !! basal topography (meter)
